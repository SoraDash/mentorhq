...
export const getBilling = async (month: string, year: string) => {
  return await lock.acquire('billing', async () => {
    try {
      ...
      const { ciApiKey, ciEmail, email } = user;
      ...
      if (!ciApiKey) {
        ...
        return {
          error: true,
          message: 'ciApiKey is required for the request',
          status: 400,
        };
      }
      ...
      while (retryCount < MAX_RETRIES && !success) {
        try {
          ...
          retryCount++;
          console.error(
            `Fetch failed. Retry count: ${retryCount}. Error:`,
            error.message,
          );
      ...
      if (!success) {
        return {
          error: true,
          message:
            'Network error. Could not fetch data after multiple attempts.',
          status: 503, // Service unavailable
        };
      }

      if (!response || !response.ok) {
        // Check if response is falsy or HTTP status code is not in the 200-299 range
