...
import {
  ...
  unassignStudent,
  updateOrCreateStudentFromGoogle,
} from '@/lib/students';
...
export const studentsFromGoogleSheets = async (): Promise<
  GoogleSheetStudent[]
> => {
  ...
  const cachedStudents = await CacheConfig.get('students', user.email);
  ...
  const fetchedStudents = await lock.acquire(
    `students:${user.email}`,
    async () => {
...
export const syncStudentsWithDatabase = async (): Promise<{
  added: string[];
  removed: string[];
  updated: string[];
}> => {
  ...
  const sheetStudentEmails = sheetStudents.map(
    (s: Student | GoogleSheetStudent) => s.email,
  );

  const newStudents = sheetStudents.filter(
    (sheetStudent: Student | GoogleSheetStudent) =>
      !currentStudentEmails.includes(sheetStudent.email),
  );
  const unassignedStudents = currentStudents.filter(
    (currentStudent: Student) =>
      !sheetStudentEmails.includes(currentStudent.email),
  );
  ...
  if (newStudents.length > 0) {
    for (const student of newStudents) {
      student.github = student.github
        ? extractGithubUsername(student.github)
        : student.github;
  ...
  for (const sheetStudent of sheetStudents) {
    if (currentStudentEmails.includes(sheetStudent.email)) {
      sheetStudent.github = sheetStudent.github
        ? extractGithubUsername(sheetStudent.github)
        : sheetStudent.github;
